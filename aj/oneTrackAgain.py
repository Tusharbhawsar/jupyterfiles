...
for link in links[0:1]:
    ...
    try:
            ...
            if ret:
                ...
                if stopFrame> frame_n>firstAppearance:
                    ...
                    for xmin,ymin,xmax,ymax in players:
                        ...
                        cv2.circle(frame,smoothTRack[frame_n -firstAppearance -1],5,(0,0,200),5)
        ...
        startingCourt = getCourt(smoothTRack[0])
        
        service_or_track = None
        if startingCourt == "bottom":
            service_or_track = check_service_or_track(smoothTRack)
            if service_or_track == "track":
                startingCourt = "top"
        elif startingCourt == "out":
            service_or_track = check_service_or_track_4out(smoothTRack)
            if service_or_track == "service":
                startingCourt = "top"
            else:
                startingCourt = "top"    #just giving as default
        elif startingCourt == "top":
            service_or_track = check_service_or_track_4top(smoothTRack)
            if service_or_track == "track":
                startingCourt = "bottom"



        if startingCourt == "bottom":
            try:
                serviceIndex = get_service_index(smoothTRack)
                print(serviceIndex)
                jsonResultFile.update({"serviceIndex":serviceIndex})
            ...
            jsonResultFile.update({"serviceIndex":serviceIndex})
        if startingCourt == "top":
            bounce_threshold = find_bounceThreshold(smoothTRack)
        else:
            bounce_threshold = 15
        bounces, turns,angles_index = get_bounce_frame(smoothTRack,bounce_threshold)
        bounces = [bounce for bounce in bounces if bounce>serviceIndex]

        actualTurns  = turns.copy()

        print(""""DETECTED""" "")

        service_Point = servicePoint(startingCourt)
        print(
            "///////////////////////////////////////////",
            service_or_track,
            startingCourt,
            service_Point,
            getCoordinateServicePosition_4(service_Point),
            "///////////////////////////////////////////",
        )
        servicePosition = getCoordinateServicePosition_4(service_Point)
        print(serviceIndex,bounces, turns)

        aceServe = False
        for index,bounce_index in enumerate(bounces):
            bounce_Court = getPosition(smoothTRack[bounce_index], None)
            print(bounce_index, bounce_Court)
            if bounce_Court == startingCourt:
                print(
                    "************************--------------------",
                    smoothTRack[bounce_index],
                    bounce_index,
                    getCoordinateServicePosition_4(smoothTRack[bounce_index]),
                    "///////////////////////////////////////////PASSING",
                )
                pass

            else:
                # baseline_top, baseline_bottom, left_court_line, right_court_line, left_inner_line, right_inner_line, middle_line, top_inner_line, bottom_inner_line, net = detectCourt(cv2.imread(os.path.join(folder,filename[bounce_index])))
                # topLeft_box_poly, topRight_box_poly, bottomLeft_box_poly, bottomRight_box_poly = get_Boxes()
                bouncePosition = getCoordinatePosition_4(
                    smoothTRack[bounce_index]
                )
                jsonResultFile.update({"bouncePosition": bouncePosition})
                ...
                if aceCourtDict[servicePosition] == bouncePosition:
                    print("ACESERVETRUE",bounces[index+1:])
                    aceServe = True
                    if index<len(bounces)-2:_ = turns.extend(bounces[index+1:])
                else:
                    ball_is_in_court = getCoordinateServicePosition_4(smoothTRack[bounce_index])
                    print(ball_is_in_court, "CHECKING IF IT TOUCHES ANY LINE")
                    if check_for_lineTouching(ball_is_in_court,smoothTRack[bounce_index]):
                        aceServe = True
                        if index<len(bounces)-2:_ = turns.extend(bounces[index+1:])

                    #handle double bounces
                    elif index<len(bounces)-1:
                        if bounces[index+1]-bounce_index <15:
                            ball_is_in_court = getCoordinateServicePosition_4(smoothTRack[bounces[index+1]])
                            print(ball_is_in_court, "CHECKING IF IT TOUCHES ANY LINE")
                            if check_for_lineTouching(ball_is_in_court,smoothTRack[bounces[index+1]]):
                                if index+1<len(bounces)-2:_ = turns.extend(bounces[index+2:])
                                aceServe = True
                            else:
                                aceServe = False
                    else:
                ...
                break
        print(aceServe)
        jsonResultFile.update({"bounces":bounces,"turns":turns, "angles_index":angles_index})
        turns = sorted(turns)
        # without racket coords
        if aceServe:
            for turn in turns:
                if turn > bounce_index+10:
                    ballReturndConfCount = 0
                    turnx, turny = smoothTRack[turn]
                    if turny > net[1]:
                        for x, y in smoothTRack[turn:]:
                            if y < net[1] + 50 and 0 < x < c_w:
                                print(
                                    f"{turn},::::ballReturnedballReturnedballReturned, {len(smoothTRack)}"
                                )
                                ballReturndConfCount += 1
                                if ballReturndConfCount >= 3:
                                    print("NOT ACE SERVE BALL RETURNEDRETURNED")
                                    break
                            else:
                                ballReturndConfCount = 0
                    else:
                        for x, y in smoothTRack[turn:]:
                            if y > net[1] - 100 and 0 < x < c_w:
                                print(
                                    f"{turn},::::ballReturnedballReturned, {len(smoothTRack)}",
                                    x,
                                    y,
                                )
                                ballReturndConfCount += 1
                                if ballReturndConfCount >= 3:
                                    print("NOT ACE SERVE RETURNEDBALL RETURNED")
                                    break
                            else:
                                ballReturndConfCount = 0
                    if ballReturndConfCount >= 3:
                        aceServe = False
                        print("NOT ACE SERVE BALL RETURNED")
                        break
        # if aceServe:
        #     for turn in actualTurns:
        #         if turn>bounce_index+10:
        #             if not checkReturnedFromRacket(allImages[firstAppearance - startAt+turn-4:firstAppearance - startAt+turn+4],smoothTRack[turn]):
        #                 continue
        #             else:
        #                 aceServe = False
        #                 print("NOT ACE SERVE BALL hit by racket")
        #                 jsonResultFile.update({"racketTurnIndex":turn})
        #                 break
        # cv2.destroyAllWindows()
        video.release()
    except:
