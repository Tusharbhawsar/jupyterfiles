...
video_url = "https://dwapv64icf8j2.cloudfront.net/3cxAmKXbu/1674964254.mp4"
...
def remove_multi_trakcs(filtered_movement_list, upto):
    ...
    # import pandas as pd
    # # x, y = [x[0] if x is not None else np.nan for x in self.actualBalls], [x[1] if x is not None else np.nan for x in self.actualBalls]
    # x = list(cords['x'])
    # y = list(cords['y'])
    ...
    for index, items in enumerate(range(start, end)):
        cord_dict[items] = [x[index], y[index], cord_dict[items][-1]]
