...
for row in rows[:500]:
    try:
        ...
        while video.isOpened():
            ...
            openResponse = requests.post(url_racket,files= file)

            if len(openResponse.json())>0:
                openResponsePerson = requests.post(url_person,files= file)
                
                if len(openResponsePerson.json())>0:

                    personList = []
                    for result in openResponsePerson.json():
                        # print(result)
                    ...
                    jsonResult_person.update({frame_n:openResponsePerson.json()})
                ...
                for result in openResponse.json():
                    ...
                    # cv2.putText(frame,str(conf),(int(xmin),int(ymin-10)),cv2.FONT_HERSHEY_SIMPLEX,.8,(0,0,255),1)
                jsonResult_racket.update({frame_n:openResponse.json()})
        ...
        # cv2.destroyAllWindows()
            ...
            with open(f"{saveFolder}/{saveFolder}_racket.json", "w") as bouncecords:
                jsondata = json.dumps(jsonResult_racket,cls = NumpyEncoder)
            ...
            with open(f"{saveFolder}/{saveFolder}_person.json", "w") as bouncecords:
                jsondata = json.dumps(jsonResult_person,cls = NumpyEncoder)
                json.dump(jsondata,bouncecords)

    except Exception as e:
        print("ERRPRPRRPRPRPR", e)

        with open(f"{saveFolder}_racket.json", "w") as bouncecords:
            jsondata = json.dumps(jsonResult_racket,cls = NumpyEncoder)
            json.dump(jsondata,bouncecords)
        ...
        with open(f"{saveFolder}_person.json", "w") as bouncecords:
            jsondata = json.dumps(jsonResult_person,cls = NumpyEncoder)
            json.dump(jsondata,bouncecords)
